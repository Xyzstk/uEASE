...
static inline void IOInit(void) {
	...
	sm_config_set_out_pins(&c, PIN_TEST, 1);
	sm_config_set_in_pins(&c, PIN_TEST);
	sm_config_set_set_pins(&c, PIN_TEST, 1);
	sm_config_set_jmp_pin(&c, PIN_TEST);
	sm_config_set_sideset_pins(&c, PIN_RESET_N);
	sm_config_set_out_shift(&c, false, false, 24);
	sm_config_set_in_shift(&c, false, true, 16);
	sm_config_set_clkdiv(&c, 16.625f);
	pio_sm_set_pindirs_with_mask(pio0, 0, (1u << PIN_RESET_N) | (1u << PIN_TEST), (1u << PIN_RESET_N) | (1u << PIN_TEST));
	pio_sm_set_pins_with_mask(pio0, 0, 1u << PIN_RESET_N, (1u << PIN_RESET_N) | (1u << PIN_TEST));
	pio_gpio_init(pio0, PIN_RESET_N);
	pio_gpio_init(pio0, PIN_TEST);
	hw_set_bits(&pio0->input_sync_bypass, 1u << PIN_TEST);
